...
class ApiService {
  ...
  static Future<List<T>> getAllItems<T>({int retries = 0, required T Function(Map<String, dynamic>) fromJson}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Error: ${response.statusCode}. Respuesta: ${response.reasonPhrase}');
  ...
  static Future<T?> getItem<T>(String itemId, T Function(Map<String, dynamic>) fromJson) async {    
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Error al obtener $T: ${response.statusCode}. Respuesta: ${response.reasonPhrase}');
      }
    } catch (e) {
      throw Exception(e);
    }
  ...
  static Future<void> updateItem<T>(T updatedItem, String updatedItemId) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        print('Error al actualizar $T: ${response.statusCode}. Respuesta: ${response.reasonPhrase}');
      }
    } catch (e) {
  ...
  static Future<void> deleteItem<T>(String itemId) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        throw Exception('Error al eliminar $T: ${response.statusCode}. Respuesta: ${response.reasonPhrase}');
      }
    } catch (e) {
  ...
  static Future<List<RestaurantData>> getFavoriteRestaurants(String user_id) async {
    Uri uri = Uri.parse('$baseUrl/rest/v1/favorite_restaurants_view?favorite_user_id=eq.$user_id');
    final headers = {
      'Content-Type': 'application/json; charset=UTF-8',
      'apikey': dotenv.env['SUPABASE_KEY'] ?? '',
      'Authorization': 'Bearer ${dotenv.env['SUPABASE_KEY'] ?? ''}',
    ...
    try {
      final response = await http.get(uri, headers: headers);
      if (response.statusCode == 200) {
        List<RestaurantData> favoriteRestaurants = (json.decode(response.body) as List)
          .map((itemJson) => RestaurantData.fromJson(itemJson as Map<String, dynamic>))
          .toList();
        print('Restaurantes de usuario $user_id obtenidos correctamente');
        return favoriteRestaurants;
      } else {
        throw Exception('Error al cargar restaurantes favoritos. ${response.reasonPhrase}');
      }
    } catch (e) {
      rethrow;
    }
  ...
  static Future<List<UserReview>?> getUserReviews(String restaurant_id) async {
    Uri uri = Uri.parse('$baseUrl/rest/v1/user_reviews?review_restaurant_id=eq.$restaurant_id');
    final headers = {
      'Content-Type': 'application/json; charset=UTF-8',
      'apikey': dotenv.env['SUPABASE_KEY'] ?? '',
      'Authorization': 'Bearer ${dotenv.env['SUPABASE_KEY'] ?? ''}',
    ...
    print('Obteniendo reseñas de restaurante: $restaurant_id y información de usuarios.');
  ...
  static Future<List<WeatherData>?> getWeather(DateTime localTime, LatLng position) async {
    const String baseWeatherUrl = 'https://api.openweathermap.org/data/3.0/onecall';
    String params = '?lat=${position.latitude}&lon=${position.longitude}&exclude=minutely,hourly&units=metric&appid=${dotenv.env['OPENWEATHERMAP_API_KEY']}&lang=es';

    print('Obteniendo informe meteorológico de [${position.latitude}, ${position.longitude}] a las $localTime');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<WeatherData> forecasts = WeatherData.fromDailyJson(decodedResponse['daily'], position);
        print('Informe meteorológico obtenido correctamente.');
        return forecasts;
      } else {
        throw Exception('Error al obtener informe meteorológico: ${response.reasonPhrase}');
      }
    } catch (e) {
      rethrow;
    }
  ...
  static const Map<Type, Map<String, String>> typeToTableData = {
    ...
    UserData: {
      'url': '/rest/v1/user_data',
      'primaryKey': 'user_data_id'
    },
    RestaurantData: {
